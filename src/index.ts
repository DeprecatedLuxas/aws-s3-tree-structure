...
function listAllObjects(s3Client: AWS.S3, objects: any, bucket: string, callback: any, token?: string) {
    ...
    s3Client.listObjectsV2(opts, function (err: any, data: any) {
        ...
        if (data.IsTruncated) {
            listAllObjects(s3Client, objects, bucket, callback, data.NextContinuationToken)
        } else {
...
(async () => {
    ...
    const bucketList: AWS.S3.Buckets = await new Promise((success, failure) => {
    ...
    let allObjects: AWS.S3.ObjectList  = await new Promise((success, failure) => {
    ...
    allObjects.map((object: AWS.S3.Object) => object.Key).map((item: any) => item.split("/")).forEach((itemParts) => {
        buildTreeStructure(itemParts)
    })
    fs.writeFileSync("./awstreestructure.json", JSON.stringify(treeStructureObject, null, 4));
